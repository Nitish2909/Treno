...
import { Plus, Pencil, Trash2, Eye, ToggleLeft, ToggleRight, Star } from 'lucide-react'
import {
  useGetAllTripsQuery,
...
export default function Trips() {
  ...
  const { data, isLoading } = useGetAllTripsQuery({
    page, limit: pageSize, search, status: statusFilter, type: typeFilter, category: catFilter,
  })
  ...
  async function handleToggleFeatured(trip) {
    try {
      ...
      toast.success(trip.featured ? 'Removed from featured' : 'Marked as featured')
    } catch (e) { toast.error(e?.data?.message || 'Update failed') }
  }
  ...
  const columns = [
    {
      ...
      render: (val, row) => (
        <img
          src={val || row.images?.[0] || 'https://placehold.co/56x40/e2e8f0/94a3b8?text=Trip'}
          alt={row.title}
    ...
    },
    {
      ...
      render: (val, row) => (
        <div>
          ...
          <p className="text-xs text-gray-400">{row.from} · {row.durationDays}D</p>
        </div>
      ),
    },
    {
      ...
      render: (val) => (
        <span className="badge bg-blue-50 text-blue-600">{typeof val === 'object' ? val?.name : val || '—'}</span>
      ),
    },
    { key: 'type', label: 'Type', render: (v) => v || '—' },
    {
      key: 'discountedPrice', label: 'Price', sortable: true,
      render: (val, row) => (
        <div>
          <p className="font-semibold">{formatPrice(val || row.originalPrice)}</p>
          {val && val < row.originalPrice && (
            <p className="text-xs text-gray-400 line-through">{formatPrice(row.originalPrice)}</p>
          )}
        </div>
      ),
    },
    {
      ...
      render: (val, row) => (
        <div className="flex items-center gap-2">
          ...
          {row.featured && <Star size={12} className="fill-amber-400 text-amber-400" />}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          ...
          >
            <Star size={14} className={row.featured ? 'fill-amber-400 text-amber-400' : ''} />
          </button>
          <button
  ...
  return (
    <>
      <Helmet><title>Trips — Treno Admin</title></Helmet>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <select className="form-select w-auto text-sm" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1) }}>
          <option value="">All Statuses</option>
          <option value="active">Active</option>
          <option value="inactive">Inactive</option>
          <option value="draft">Draft</option>
        </select>
        <select className="form-select w-auto text-sm" value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(1) }}>
          <option value="">All Types</option>
          {TRIP_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
        </select>
        <select className="form-select w-auto text-sm" value={catFilter} onChange={(e) => { setCatFilter(e.target.value); setPage(1) }}>
      ...
      <DataTable
        ...
        bulkActions={[{ label: 'Delete Selected', onClick: handleBulkDelete, variant: 'danger', icon: Trash2 }]}
        searchable
