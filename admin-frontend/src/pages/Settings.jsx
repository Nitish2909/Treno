...
import {
  User, Lock, Bell, Camera, Loader2, Eye, EyeOff, Check,
} from 'lucide-react'
import {
...
export default function Settings() {
  ...
  const [updateProfile, { isLoading: profileLoading }] = useUpdateAdminProfileMutation()
  ...
  const [changePassword, { isLoading: pwLoading }] = useChangeAdminPasswordMutation()
  ...
  async function handlePasswordChange() {
    ...
    } catch (e) { toast.error(e?.data?.message || 'Failed to change password') }
  }
  ...
  return (
    <>
      <Helmet><title>Settings — Treno Admin</title></Helmet>
      <PageHeader
        title="Settings"
        ...
        <div className="flex-1">
          ...
          {tab === 'profile' && (
            <div className="card card-body space-y-5 max-w-xl">
              ...
              <div>
                ...
                <input className="form-input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@Treno.in" />
              </div>
          ...
          {tab === 'password' && (
            <div className="card card-body space-y-5 max-w-md">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={clsx('form-input pr-10', pwErrors.currentPw && 'error')}
                    value={currentPw}
                ...
                {pwErrors.currentPw && <p className="form-error">{pwErrors.currentPw}</p>}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={clsx('form-input pr-10', pwErrors.newPw && 'error')}
                    value={newPw}
                ...
                {pwErrors.newPw && <p className="form-error">{pwErrors.newPw}</p>}
              </div>
              ...
              <div>
                ...
                />
                {pwErrors.confirmPw && <p className="form-error">{pwErrors.confirmPw}</p>}
              </div>
              ...
              {newPw && (
                <div>
                  <div className="flex gap-1 mb-1">
                    {[1,2,3,4].map((s) => (
                      <div key={s} className={clsx('h-1.5 flex-1 rounded-full transition-colors', {
                        'bg-danger-500':  newPw.length >= s * 2 && newPw.length < 8,
                        'bg-warning-500': newPw.length >= 8 && newPw.length < 12,
          ...
          {tab === 'notifications' && (
            <div className="card card-body space-y-5 max-w-lg">
              ...
              <p className="text-sm text-gray-500">Choose which events you want to be notified about.</p>

              <div className="space-y-4">
                {[
                  { key: 'newBooking',   label: 'New Booking',   desc: 'When a new booking is made'     },
                  { key: 'newUser',      label: 'New User',      desc: 'When a new user registers'      },
                  { key: 'cancellation', label: 'Cancellation',  desc: 'When a booking is cancelled'    },
                  { key: 'reviews',      label: 'New Review',    desc: 'When a new review is submitted' },
                ].map((n) => (
              ...
              <div className="flex justify-end">
                <button className="btn btn-primary" onClick={handleSaveNotifications} disabled={profileLoading}>
                  {profileLoading ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}
                  Save Preferences
                </button>
              </div>
