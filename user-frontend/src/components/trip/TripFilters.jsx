    ...
    <div className="space-y-3">
      <div className="relative h-6 flex items-center">
        ...
        <input
          ...
          value={min}
          onChange={handleMin}
          className="absolute w-full h-full appearance-none bg-transparent pointer-events-none z-10 cursor-pointer
        ...
        <input
          ...
          value={max}
          onChange={handleMax}
          className="absolute w-full h-full appearance-none bg-transparent pointer-events-none z-20 cursor-pointer
            [&::-webkit-slider-thumb]:pointer-events-auto
            [&::-webkit-slider-thumb]:appearance-none
            [&::-webkit-slider-thumb]:w-4
            [&::-webkit-slider-thumb]:h-4
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-sm transition-colors ${
                        checked ? 'text-amber-600 font-medium' : 'text-gray-600'
                      }`}
                    >
                      {label}
                    </span>
        ...
        <FilterSection title="Duration">
          <div className="space-y-2">
            {DURATIONS.map(({ label, value }) => {
              const checked = durations.includes(value);
              return (
                <label key={value} className="flex items-center gap-2 cursor-pointer group select-none">
                  <div
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all duration-150
                      ${
                        checked
                  ...
                  <input
                    ...
                    className="sr-only"
                    checked={checked}
                    onChange={() => dispatch(toggleDuration(value))}
                  />
                  <span className={`text-sm ${checked ? 'text-amber-600 font-medium' : 'text-gray-600'}`}>
                    {label}
                  </span>
                </label>
              );
              ...
              return (
                <label key={value} className="flex items-center gap-2 cursor-pointer group select-none">
                  ...
                  <input
                    ...
                    className="sr-only"
                    checked={checked}
                    onChange={() => dispatch(toggleDifficulty(value))}
                  />
          ...
          <div className="flex items-center gap-1">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                ...
                key={star}
                onClick={() =>
                  dispatch(setFilter({ key: 'rating', value: star === minRating ? 0 : star }))
                }
                className="transition-transform hover:scale-110 focus:outline-none"
                aria-label={`Set minimum rating ${star}`}
              >
