...
// Safe date parser for react-datepicker
const parseValidDate = (dateStr) => {
  if (!dateStr || dateStr === "custom") return null;
...
function validateStep1(form) {
  const errs = {};
  if (!form.startDate || form.startDate === "custom") {
    errs.startDate = "Please select a valid departure date.";
  }
  if (!form.travelers || form.travelers < 1) {
    errs.travelers = "At least 1 traveler required.";
  }
  return errs;
}
...
function validatePassenger(p) {
  ...
  if (!p.name || p.name.trim().length < 3)
    e.name = "Full name is required (min 3 chars).";
  const age = parseInt(p.age, 10);
  if (!p.age || isNaN(age) || age < 2 || age > 99)
    e.age = "Age must be between 2 and 99.";
  ...
  if (!p.idNumber || p.idNumber.trim().length < 4)
    e.idNumber = "Please enter a valid ID number.";
  return e;
}
...
function validateStep3(form) {
  ...
  if (
    ...
    form.emergencyRelationship.trim().length < 2
  )
...
export default function BookingForm({ trip, onComplete }) {
  ...
  const isCustomDateSelected =
    ...
    startDate !== "custom" &&
    hasDates &&
    !trip.startDates.some((d) => d.date === startDate);
        ...
        return (
          <div className="space-y-5">
            ...
            <p className="text-sm text-slate-500">
              Fill in the details for all {travelers} traveler
              {travelers > 1 ? "s" : ""} as per their ID proof.
            </p>
            {passengers.map((p, i) => (
              <PassengerForm
                key={i}
                  ...
                  <div
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center transition ${
                      termsAccepted
                        ...
                        : "border-slate-400 group-hover:border-amber-400"
                    }`}
                  >
                    {termsAccepted && (
                      <svg
