    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-8 font-sans antialiased">
      ...
      <div className="relative rounded-3xl overflow-hidden shadow-2xl min-h-[380px] md:min-h-[420px] flex items-center">
        ...
        <div className="relative z-10 p-8 md:p-14 max-w-2xl text-white space-y-4">
          ...
          <p className="text-gray-200 text-base md:text-lg font-light tracking-wide">
            Where Forever Begins... Together! Craft unforgettable memories with tailored luxury getaways.
          </p>

          <div className="pt-2">
            <button className="px-7 py-3.5 bg-amber-400 hover:bg-amber-300 text-slate-900 font-bold rounded-xl shadow-lg shadow-amber-400/20 hover:shadow-amber-400/40 transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0">
              Explore Packages
            </button>
          </div>
        </div>
      ...
      <div className="relative -mt-20 md:-mt-24 z-20 px-2 md:px-6">
        ...
        <div
          ...
          className="flex gap-5 overflow-x-auto scrollbar-hide scroll-smooth py-4 px-2"
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {destinations.map((place) => (
            <div
              key={place.id}
              className="group relative flex-none w-[220px] md:w-[240px] h-[340px] md:h-[380px] rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer border border-white/20"
            >
              {/* Card Image */}
              <img
                src={place.image}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5 z-10 text-white transform transition-transform duration-300 group-hover:-translate-y-1">
                ...
                <div className="mt-2 pt-2 border-t border-white/20 flex justify-between items-end">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-300 font-medium">Starting From</p>
                    <p className="text-lg font-extrabold text-white">{place.price}</p>
                  </div>
                </div>
              </div>
            </div>
