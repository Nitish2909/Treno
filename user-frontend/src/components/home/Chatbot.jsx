...
const STEP_OPTIONS = {
  month: [
    ...
    "May 2027", "June 2027", "July 2027", "Not sure"
  ],
  duration: [
    "1–3 days", "4–7 days", "8–12 days", "12+ days"
  ],
  budget: [
    "Under ₹25k", "₹25k – ₹50k", "₹50k – ₹1L", "Above ₹1L"
  ],
  accommodation: [
    "3★ Budget", "4★ Comfort", "5★ Luxury", "Homestay / Resort"
  ],
  flights: ["Yes", "No"],
  pickup: ["Yes", "No"],
  groupType: ["Corporate Team Outing", "Family Get-together", "Friends Group", "College Trip"]
};
...
const FLOW_CONFIGS = {
  "Plan a Customized Trip": [
    ...
    { step: 'destination', prompt: 'Which destination are you interested in? (e.g. Bali, Maldives, Manali, Goa)' },
    { step: 'travellers', prompt: 'How many people will be travelling? (e.g. 2 adults, 1 child)' },
    { step: 'children', prompt: "Will any children be joining? If yes, please mention their ages. If no, just type 'No'." },
    ...
    { step: 'accommodation', prompt: 'What type of accommodation do you prefer?' },
    { step: 'flights', prompt: 'Do you need us to arrange flights as well?' },
    { step: 'pickup', prompt: 'Would you like pickup and drop services included?' },
    { step: 'departureCity', prompt: 'Which city will you be departing from?' },
    { step: 'phone', prompt: "Last step — what's your WhatsApp number so our travel expert can reach you?" }
  ],
  "Browse Packages": [
    { step: 'name', prompt: 'Please type your name.' },
    { step: 'destination', prompt: 'Which destination packages would you like to browse? (e.g. Europe, Thailand, Himachal)' },
    ...
    { step: 'departureCity', prompt: 'Which city will you be departing from?' },
    { step: 'phone', prompt: "Please share your WhatsApp number so we can send curated package itineraries directly to you!" }
  ],
  "Honeymoon Packages": [
    { step: 'name', prompt: 'Congratulations! Please type your name.' },
    { step: 'destination', prompt: 'Which romantic destination do you have in mind? (e.g. Maldives, Bali, Kashmir, Switzerland)' },
    { step: 'month', prompt: 'Which month is your wedding/honeymoon trip planned for?' },
    { step: 'duration', prompt: 'How many days are you planning for your getaway?' },
    { step: 'budget', prompt: 'What is your expected package budget per couple?' },
    { step: 'accommodation', prompt: 'What style of stay do you prefer for your honeymoon?' },
    { step: 'flights', prompt: 'Would you like us to include round-trip flights?' },
    { step: 'departureCity', prompt: 'Which city will you be flying out from?' },
    { step: 'phone', prompt: "Please share your WhatsApp number so our romantic travel specialist can reach out with tailored packages!" }
  ],
  "Group / Corporate Trip": [
    { step: 'name', prompt: 'Please enter your name / contact person name.' },
    { step: 'companyName', prompt: 'What is the name of your organization or group?' },
    { step: 'groupType', prompt: 'What type of group trip is this?' },
    { step: 'groupSize', prompt: 'How many people are expected in the group?' },
    { step: 'destination', prompt: 'Any specific destination in mind, or are you looking for recommendations?' },
    ...
    { step: 'pickup', prompt: 'Do you require team transportation/coaches arranged?' },
    { step: 'phone', prompt: 'What is your phone number so our corporate account manager can contact you?' }
  ]
};
...
export default function Chatbot() {
  ...
  const processFlowStep = (userValue, selectedFlowKey = null) => {
    ...
    if (nextIndex < flowSteps.length) {
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          { id: Date.now() + 1, sender: 'bot', text: "Thank you! Our travel expert will contact you shortly with personalized details. 🚀" }
        ]);
      }, 300);
    }
  };
  ...
  const sendMessage = async (textToSend) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, sender: 'bot', text: data.text || "Sorry, I couldn't process that request." }
      ]);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, sender: 'bot', text: "Error connecting to server." }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      {!isOpen && (
        ...
        >
          <img src={AVATAR_URL} alt="Ira" className="w-12 h-12 rounded-full object-cover" />
          <span className="text-[10px] font-bold text-white bg-black/40 px-2 py-0.5 rounded-full mt-0.5">
            Talk With Ira
          </span>
        </button>
      )}
        ...
        <div className="w-[360px] sm:w-[380px] h-[550px] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border  border-gray-100">
          ...
          <div className="bg-[#1e5629] px-5 py-4 flex items-center justify-between text-white">
            <div className="flex items-center gap-3">
              <img src={AVATAR_URL} alt="Ira" className="w-10 h-10 rounded-full object-cover border border-white/20" />
              <div>
