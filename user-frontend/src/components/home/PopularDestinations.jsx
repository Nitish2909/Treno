...
// Memoized card with 3D tilt effect applied via Framer Motion transform properties
const DestinationCard = React.memo(function DestinationCard({ dest, large = false }) {
  const navigate = useNavigate();
  ...
  return (
    <div style={{ perspective: 1000 }} className={large ? 'col-span-1 sm:col-span-2 row-span-2' : ''}>
      <motion.div
        // Handle mount animation explicitly per card so dynamically loaded pages remain visible
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        ...
        onClick={handleNavigate}
        className={`group relative overflow-hidden rounded-3xl cursor-pointer bg-slate-900 shadow-lg hover:shadow-2xl transition-shadow duration-300 h-full ${
          large
